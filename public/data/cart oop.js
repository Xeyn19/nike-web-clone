class Cart {
  ...
  addToCart(shoeId){
    let matchingShoe = this.cartItems.find(cartItem => cartItem.id === shoeId);
  ...
  removeFromCart(shoeId){
    this.cartItems = this.cartItems.filter(cartItem => cartItem.id !== shoeId );
    this.saveToStorage();
  }

  updateQuantity(shoeId , inputValue){
    let machingItem = this.cartItems.find(cartItem => cartItem.id === shoeId);
...
export function showAddToCartModal(shoe) {
  ...
  const beautyProducts = new Cart('cart-bp');
